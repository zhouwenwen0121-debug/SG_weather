...
export const HazeSection: React.FC<HazeSectionProps> = ({ hazeData }) => {
  ...
  const getPsiBg = (psi: number | null) => {
    if (!psi) return 'bg-slate-800 border-slate-700 text-slate-300';
    if (psi <= 50) return 'bg-emerald-950/40 border-emerald-500/40 text-emerald-200';
    if (psi <= 100) return 'bg-blue-950/40 border-blue-500/40 text-blue-200';
    if (psi <= 200) return 'bg-amber-950/40 border-amber-500/40 text-amber-200';
    if (psi <= 300) return 'bg-orange-950/40 border-orange-500/40 text-orange-200';
    ...
    <section id="haze-section" className="space-y-6">
      ...
      >
        <div className="flex items-start gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-300 mt-1">
              {overallAdvisory} • 1-Hour PM2.5: <strong className="text-white">{overallPm25} µg/m³</strong> ({overallPm25Band})
            </p>
        ...
        <div className="flex items-center gap-4 text-xs shrink-0 bg-slate-900/60 p-3 rounded-xl border border-slate-800">
          <div>
            <span className="text-slate-400 block text-[10px]">Peak 24-Hr PSI</span>
            <span className="text-lg font-bold text-white">{overallPsi}</span>
          </div>
          <div className="w-px h-7 bg-slate-800" />
          <div>
            <span className="text-slate-400 block text-[10px]">Peak 1-Hr PM2.5</span>
            <span className="text-lg font-bold text-emerald-400">{overallPm25} <span className="text-[10px] text-slate-400">µg/m³</span></span>
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3.5">
          {regions.map((region) => (
            <div
              key={region.id}
              className={`p-4 rounded-2xl border transition-all ${getPsiBg(region.psi)}`}
            >
              ...
              <div className="mt-3 pt-2 border-t border-slate-700/50 space-y-1.5 text-xs">
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 text-[11px]">1-Hr PM2.5</span>
                  <span className="font-semibold text-white">
                    {region.pm25OneHourly ? `${region.pm25OneHourly} µg/m³` : '--'}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-slate-400 text-[11px]">Band</span>
              ...
              {showSubIndices && (
                <div className="mt-3 pt-2 border-t border-slate-700/50 space-y-1 text-[10px] text-slate-300">
                  <div className="flex justify-between">
                    <span className="text-slate-400">24-Hr PM10</span>
                    <span className="font-mono">{region.subIndices.pm10TwentyFourHourly ?? '--'} µg/m³</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">8-Hr Ozone (O3)</span>
                    <span className="font-mono">{region.subIndices.o3EightHourMax ?? '--'} µg/m³</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">1-Hr NO2</span>
                    <span className="font-mono">{region.subIndices.no2OneHourMax ?? '--'} µg/m³</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">24-Hr SO2</span>
                    <span className="font-mono">{region.subIndices.so2TwentyFourHourly ?? '--'} µg/m³</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400">8-Hr CO</span>
                    <span className="font-mono">{region.subIndices.coEightHourMax ?? '--'} mg/m³</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 shadow-lg text-xs">
          ...
          <div className="space-y-1.5">
            {scales.psi.map((item) => (
              <div key={item.range} className="flex items-center justify-between p-2 rounded-lg bg-slate-800/50">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-slate-200">{item.range}</span>
                  <span className={`text-[10px] px-1.5 py-0.2 rounded font-semibold border ${getPsiBadge(item.category)}`}>
                    {item.category}
                  </span>
                </div>
                <span className="text-slate-400 text-[11px] truncate max-w-[180px]">{item.description}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800 shadow-lg text-xs">
          ...
          <div className="space-y-1.5">
            {scales.pm25.map((item) => (
              <div key={item.band} className="flex items-center justify-between p-2 rounded-lg bg-slate-800/50">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-slate-200 text-[11px]">{item.band}</span>
