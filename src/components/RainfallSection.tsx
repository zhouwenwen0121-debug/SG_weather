  ...
  if (!rainData) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 text-center text-slate-500 dark:text-slate-400 shadow-md">
        <Radio className="w-8 h-8 text-blue-500 dark:text-blue-400 animate-pulse mx-auto mb-2" />
    ...
    <section id="rain-section" className="space-y-6">
      ...
      <div
        className={`p-4 sm:p-5 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between gap-4 transition-all shadow-sm ${
          isRaining && activeStations.length > 0
            ? 'bg-blue-50 dark:bg-blue-950/40 border-blue-200 dark:border-blue-500/40 text-blue-900 dark:text-blue-200'
      ...
      >
        <div className="flex items-start gap-3.5">
          <div
            className={`p-2.5 rounded-xl border mt-0.5 ${
              isRaining && activeStations.length > 0
                ? 'bg-blue-500/10 dark:bg-blue-500/20 border-blue-300 dark:border-blue-500/40 text-blue-600 dark:text-blue-300'
        ...
        <div className="flex items-center gap-4 text-xs shrink-0 bg-white/90 dark:bg-slate-900/60 p-3 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs">
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[10px]">Raining Stations</span>
          ...
          <div>
            <span className="text-slate-500 dark:text-slate-400 block text-[10px]">Peak 5-Min Rate</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-md flex flex-col justify-between transition-colors">
          <div>
            ...
            <div className="space-y-2.5 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-semibold text-slate-800 dark:text-white">No Significant Rain</span>
                </div>
                <span className="font-mono text-slate-600 dark:text-slate-300">0.0 mm/5m (0 mm/h)</span>
              </div>

              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-cyan-500 dark:bg-cyan-400"></span>
                  <span className="font-semibold text-slate-800 dark:text-white">Light Rain</span>
                </div>
                <span className="font-mono text-cyan-700 dark:text-cyan-300 font-semibold">&le; 0.2 mm/5m (&lt; 2.5 mm/h)</span>
              </div>

              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-blue-500"></span>
                  <span className="font-semibold text-slate-800 dark:text-white">Moderate Rain</span>
                </div>
                <span className="font-mono text-blue-700 dark:text-blue-300 font-semibold">0.3 - 0.8 mm/5m (2.5 - 10 mm/h)</span>
              </div>

              <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/60 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-pink-500"></span>
                  <span className="font-semibold text-slate-800 dark:text-white">Heavy Rain</span>
                </div>
                <span className="font-mono text-pink-700 dark:text-pink-300 font-semibold">&gt; 0.8 mm/5m (&gt; 10 mm/h)</span>
              </div>
            </div>
          </div>

          <div className="mt-5 p-3 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/30 text-[11px] text-slate-600 dark:text-slate-300 flex items-start gap-2">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl p-5 border border-slate-200 dark:border-slate-800 shadow-md flex flex-col justify-between transition-colors">
          <div>
              ...
              {displayStations.map((station) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-xl border flex items-center justify-between transition-all cursor-pointer hover:border-blue-400 dark:hover:border-blue-500/60 ${
                      hasRain
                        ? 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-500/40 text-blue-900 dark:text-blue-200'
                        : 'bg-slate-50 dark:bg-slate-800/40 border-slate-200 dark:border-slate-700/50 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/80'
                    }`}
                  >
                    <div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-mono text-slate-600 dark:text-slate-400 bg-slate-200 dark:bg-slate-800 px-1 rounded font-semibold">
                          {station.id}
                        </span>
                        <h4 className="text-xs font-semibold text-slate-900 dark:text-white truncate max-w-[140px]">
                          {station.name}
                        </h4>
                      </div>
                    ...
                    <div className="text-right">
                      <div className="text-sm font-extrabold text-slate-900 dark:text-white">
                        {station.rainfall.toFixed(1)} <span className="text-[10px] text-slate-500 dark:text-slate-400 font-normal">mm</span>
                      </div>
                      <span
                        className={`text-[9px] px-1.5 py-0.5 rounded font-bold inline-block mt-0.5 ${
          ...
          {activeForecastAreas.length > 0 && (
            <div className="mt-4 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 text-xs text-slate-700 dark:text-slate-300">
              ...
              <div className="flex flex-wrap gap-1.5 mt-1.5">
                {activeForecastAreas.slice(0, 10).map((a) => (
                  <span
                    key={a.area}
                    className="px-2 py-0.5 rounded bg-blue-50 dark:bg-blue-500/20 border border-blue-200 dark:border-blue-500/30 text-blue-700 dark:text-blue-300 text-[11px] font-medium"
                  >
                    {a.area}: {a.forecast}
                  </span>
                ))}
      ...
      <div className="p-4 rounded-2xl bg-gradient-to-r from-slate-50 via-blue-50/50 to-slate-50 dark:from-slate-900 dark:via-blue-950/20 dark:to-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm transition-colors">
        <div>
        ...
        <a
          ...
          className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-blue-600 text-white hover:bg-blue-500 dark:bg-blue-600/20 dark:hover:bg-blue-600/30 dark:border dark:border-blue-500/40 dark:text-blue-300 dark:hover:text-white text-xs font-medium transition-all shrink-0 cursor-pointer shadow-xs"
        >
